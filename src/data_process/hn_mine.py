...
class DataArgs:
    ...
    search_batch_size: int = field(
        default=128, metadata={"help": "The batch size for searching."}
    )
...
def create_index(embeddings: np.ndarray, use_gpu: bool = False):
    ...
    if use_gpu:
        # 部分环境仅有 CPU 版 faiss，没有 GpuMultipleClonerOptions
        if hasattr(faiss, "GpuMultipleClonerOptions") and hasattr(faiss, "index_cpu_to_all_gpus"):
            co = faiss.GpuMultipleClonerOptions()
            co.shard = True
            co.useFloat16 = True
            index = faiss.index_cpu_to_all_gpus(index, co=co)
        else:
            print("faiss GPU 接口不可用，自动回退到 CPU 索引。")
...
def _to_dense_array(embeddings):
    """
    Normalize embedder outputs to 2D numpy array.
    Supports FlagEmbedding multi-vector outputs (e.g., {'dense_vecs': np.ndarray}).
    """
    ...
    # For multi-vector outputs with shape (batch, m, dim), average over m
...
def find_knn_neg(
    ...
    if len(corpus) == 0:
        raise ValueError("Corpus is empty, cannot mine hard negatives.")

    print(f'inferencing embedding for corpus (number={len(corpus)})--------------')
    p_vecs = _to_dense_array(model.encode(corpus))
    print(f'inferencing embedding for queries (number={len(queries)})--------------')
    q_vecs = _to_dense_array(model.encode_queries(queries))
